...
def admin_dashboard(request):
    ...
    for employee in employees:
        ...
        pending_calls = HappyCall.objects.filter(
            Q(first_call_caller=employee.user, call_stage__in=['1st_pending', '1st_in_progress']) |
            Q(second_call_caller=employee.user, call_stage__in=['2nd_pending', '2nd_in_progress']) |
            Q(third_call_caller=employee.user, call_stage__in=['3rd_pending', '3rd_in_progress']) |
            Q(fourth_call_caller=employee.user, call_stage__in=['4th_pending', '4th_in_progress'])
        ).distinct().count()
        
        # 완료된 해피콜 (이번 주)
        completed_calls = HappyCall.objects.filter(
            Q(first_call_caller=employee.user, first_call_success=True, first_call_date__gte=week_ago) |
            Q(second_call_caller=employee.user, second_call_success=True, second_call_date__gte=week_ago) |
            Q(third_call_caller=employee.user, third_call_success=True, third_call_date__gte=week_ago) |
            Q(fourth_call_caller=employee.user, fourth_call_success=True, fourth_call_date__gte=week_ago)
        ).distinct().count()
        
        # 전체 담당 해피콜
        total_assigned = HappyCall.objects.filter(
    ...
    # 서비스 타입별 통계 (이번 달)
    service_type_stats = ServiceRequest.objects.filter(
    ...
    ).select_related('assignee', 'department').order_by('start_datetime')[:10]
    ...
    templates = {
        'customers': {
            ...
            'headers': [
                ...
                'customer_type', 'membership_status', 'customer_grade', 
                'business_number', 'company_name', 'privacy_consent', 
                'marketing_consent', 'do_not_contact', 'is_banned', 'notes'
            ],
            'sample_data': [
                ['홍길동', '010-1234-5678', 'hong@example.com', '서울시 강남구', '테헤란로 123',
                 'individual', 'basic', 'A', '', '', 'TRUE', 'FALSE', 'FALSE', 'FALSE', '샘플 고객 데이터']
            ]
        },
        'vehicles': {
        ...
        'services': {
            ...
            'headers': [
                ...
                'status', 'priority', 'description', 'estimated_price', 'notes'
            ],
            'sample_data': [
                ['010-1234-5678', '123가4567', 'engine_oil', '2024-01-15',
                 'completed', 'normal', '엔진오일 교환', '50000', '샘플 서비스 데이터']
            ]
        }
    }
